...
public class Player extends Creature{
	...
	private void checkEncounter() {
		...
		if((w.getTile(w.getSpawnX() + ((int)Creature.xPosition) / 64, 
			w.getSpawnY() + ((int)Creature.yPosition) / 64) == Tile.bush) && !BattleState.encounterFlag && Math.random() >= 0.99) {
			...
			if(a != w.getSpawnX() + ((int)Creature.xPosition) / 64 || b != w.getSpawnY() + ((int)Creature.yPosition) / 64) {
